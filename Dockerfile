...
RUN [ -s /etc/ssl/public.key ] || { echo "❌ Ошибка: Файл cert.pem пустой или не существует!"; exit 1; }
RUN [ -s /etc/ssl/private.key ] || { echo "❌ Ошибка: Файл key.pem пустой или не существует!"; exit 1; }
